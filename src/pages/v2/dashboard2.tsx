  ...
  return (
    <div className="flex flex-col gap-4 mt-2 w-full h-full">
      ...
      <div className="flex flex-row gap-6 mt-4">
        {userDummy.map((user) => (
          <div key={user.id} className="flex flex-col gap-2 p-4 border rounded-md w-1/3">
            <h1 className="font-semibold text-lg">{user.name}</h1>
            <p className="text-sm">Email: {user.email}</p>
            <button className="bg-red-500 hover:bg-red-400 mt-2 px-4 py-2 rounded-md hover:font-semibold">
              Revoke
            </button>
          </div>
        ))}
